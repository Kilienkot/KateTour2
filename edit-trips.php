        ...
        for ($i = 1; $i <= 5; $i++) {
            if (isset($_FILES["photo_$i"]) && $_FILES["photo_$i"]['error'] == 0) {
                ...
                if (resizeImage($file['tmp_name'], $filepath)) {
                    ...
                    // Обновить или создать фото
                    $new_size = filesize($filepath);
                    
                    // Проверяем существует ли уже запись для этой позиции
                    $stmt = $pdo->prepare("SELECT id FROM tour_photos WHERE tour_id = ? AND sort_order = ?");
                    $stmt->execute([$tour_id, $i]);
                    $exists = $stmt->fetchColumn();
                    
                    if ($exists) {
                        // Если запись есть - обновляем
                        $stmt = $pdo->prepare("UPDATE tour_photos SET filename = ?, original_filename = ?, filepath = ?, file_size = ?, mime_type = ?, is_primary = ? WHERE tour_id = ? AND sort_order = ?");
                        $stmt->execute([$filename, $file['name'], $filepath, $new_size, $file['type'], $i == 1 ? 1 : 0, $tour_id, $i]);
                    } else {
            ...
            <?php if ($edit_tour): ?>
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="full_description" name="full_description" rows="5" maxlength="1500" placeholder="Приглашаем тебя с нами в захватывающее путешествие..." required><?php echo htmlspecialchars($edit_tour['full_description']); ?></textarea>
                    </div>
